...
printVolInfo() {
	...
	if [[ $Mute == "[off]" ]]; then
		textBox "^ca(1,$VOL_TOGGLE_CMD)^ca(4,$VOL_UP_CMD)^ca(5,$VOL_DOWN_CMD)VOLUME^ca()^ca()^ca() ^fg()${Perc}" "" ${CRIT2}
	else
		textBox "^ca(1,$VOL_TOGGLE_CMD)^ca(4,$VOL_UP_CMD)^ca(5,$VOL_DOWN_CMD)VOLUME^ca()^ca()^ca() ^fg(${CRIT2})${Perc}" ${DZEN_FG2} ${DZEN_BG2}
	fi
	return
}
...
printDropBoxInfo() {
	...
	if [[ $DropboxON == "0" ]]; then
		textBox "^ca(1,$DROP_START_CMD)DROPBOX^ca() ^fg()Off" ${DZEN_FG2} ${DZEN_BG2}
	else
		textBox "^ca(1,$DROP_STOP_CMD)DROPBOX^ca() ^fg($CRIT)On" ${DZEN_FG2} ${DZEN_BG2}
	fi
	return
}
...
printMpdInfo() {
	...
	if [[ $MPDON == "0" ]]; then
		...
		textBox "^ca(1,$MPD_TOGGLE_CMD)^ca(3,$MPD_STOP_CMD)^ca(4,$MPD_NEXT_CMD)^ca(5,$MPD_PREV_CMD)MPD^ca()^ca()^ca()^ca() $MpdInfo" ${DZEN_FG2} ${DZEN_BG2}
	fi
	return
}
