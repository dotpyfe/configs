...
myManageHook = (composeAll . concat $
	...
	where
		...
		myCodeS         = ["NetBeans IDE 7.1.2"]
		myGfxS          = ["Gimp", "gimp", "GIMP"]
		myChatS         = ["Pidgin", "Xchat"]
		myGameS         = ["zsnes"]
		myOtherS        = ["Amule", "Transmission-gtk"]
		myFloatCC       = ["MPlayer", "File-roller", "zsnes", "Gcalctool", "Exo-helper-1", "Gksu", "PSX", "Galculator", "Nvidia-settings", "XFontSel", "XCalc", "XClock", "Desmume", "Ossxmix", "Xvidcap", "Main", "Wicd-client.py", "com-mathworks-util-PostVMInit", "MATLAB"]
...
myWorkspaceBar    = "dzen2 -x '0' -y '754' -h '12' -w '946' -ta 'l' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -p -e ''"
...
myLogHook h = dynamicLogWithPP $ defaultPP
	...
	, ppSep             = ""
	, ppWsSep           = ""
	, ppCurrent         = wrapTextBox colorBlueAlt colorBlue
	, ppUrgent          = wrapTextBox colorBlack colorGreen . wrapClickWorkSpace . (\a -> (a,a))
	, ppVisible         = wrapTextBox colorBlack colorGrayAlt . wrapClickWorkSpace . (\a -> (a,a))
	, ppHiddenNoWindows = wrapTextBox colorBlack colorGrayAlt . wrapClickWorkSpace . (\a -> (a,a))
	, ppHidden          = wrapTextBox colorWhiteAlt colorGrayAlt . wrapClickWorkSpace . (\a -> (a,a))
	, ppLayout          = wrapClickLayout . wrapTextBox colorGray colorGrayAlt . wrap "LAYOUT " "" . layoutText
	, ppTitle           = wrapClickTitle . wrapTextBox colorGray colorGrayAlt . wrap "TITLE " "" . titleText
	}
	where
		--display config
		orderText (ws:l:t:_) = [ws,l,t]
		titleText [] = "^fg(" ++ colorWhiteAlt ++ ")Desktop"
		titleText x  = "^fg(" ++ colorWhiteAlt ++ ")" ++ (shorten 92 x)
		layoutText "Minimize T"  = "^fg(" ++ colorBlue ++ ")ReTall"
		layoutText "Minimize O"  = "^fg(" ++ colorBlue ++ ")OneBig"
		layoutText "Minimize TS" = "^fg(" ++ colorBlue ++ ")Tabbed"
		layoutText "Minimize TM" = "^fg(" ++ colorBlue ++ ")Master"
		layoutText "Minimize M"  = "^fg(" ++ colorBlue ++ ")Mosaic"
		layoutText "Minimize MT" = "^fg(" ++ colorBlue ++ ")Mirror"
		layoutText "Minimize G"  = "^fg(" ++ colorBlue ++ ")Mosaic"
		layoutText "Minimize C"  = "^fg(" ++ colorBlue ++ ")Mirror"
		layoutText "Minimize ReflectX T"  = "^fg(" ++ colorGreen ++ ")ReTall X"
		layoutText "Minimize ReflectX O"  = "^fg(" ++ colorGreen ++ ")OneBig X"
		layoutText "Minimize ReflectX TS" = "^fg(" ++ colorGreen ++ ")Tabbed X"
		layoutText "Minimize ReflectX TM" = "^fg(" ++ colorGreen ++ ")Master X"
		layoutText "Minimize ReflectX M"  = "^fg(" ++ colorGreen ++ ")Mosaic X"
		layoutText "Minimize ReflectX MT" = "^fg(" ++ colorGreen ++ ")Mirror X"
		layoutText "Minimize ReflectX G"  = "^fg(" ++ colorGreen ++ ")Mosaic X"
		layoutText "Minimize ReflectX C"  = "^fg(" ++ colorGreen ++ ")Mirror X"
		layoutText "Minimize ReflectY T"  = "^fg(" ++ colorGreen ++ ")ReTall Y"
		layoutText "Minimize ReflectY O"  = "^fg(" ++ colorGreen ++ ")OneBig Y"
		layoutText "Minimize ReflectY TS" = "^fg(" ++ colorGreen ++ ")Tabbed Y"
		layoutText "Minimize ReflectY TM" = "^fg(" ++ colorGreen ++ ")Master Y"
		layoutText "Minimize ReflectY M"  = "^fg(" ++ colorGreen ++ ")Mosaic Y"
		layoutText "Minimize ReflectY MT" = "^fg(" ++ colorGreen ++ ")Mirror Y"
		layoutText "Minimize ReflectY G"  = "^fg(" ++ colorGreen ++ ")Mosaic Y"
		layoutText "Minimize ReflectY C"  = "^fg(" ++ colorGreen ++ ")Mirror Y"
		layoutText "Minimize ReflectX ReflectY T"  = "^fg(" ++ colorRed ++ ")ReTall XY"
		layoutText "Minimize ReflectX ReflectY O"  = "^fg(" ++ colorRed ++ ")OneBig XY"
		layoutText "Minimize ReflectX ReflectY TS" = "^fg(" ++ colorRed ++ ")Tabbed XY"
		layoutText "Minimize ReflectX ReflectY TM" = "^fg(" ++ colorRed ++ ")Master XY"
		layoutText "Minimize ReflectX ReflectY M"  = "^fg(" ++ colorRed ++ ")Mosaic XY"
		layoutText "Minimize ReflectX ReflectY MT" = "^fg(" ++ colorRed ++ ")Mirror XY"
		layoutText "Minimize ReflectX ReflectY G"  = "^fg(" ++ colorRed ++ ")Mosaic XY"
		layoutText "Minimize ReflectX ReflectY C"  = "^fg(" ++ colorRed ++ ")Mirror XY"
		layoutText x = "^fg(" ++ colorRed ++ ")"++x
		--box config
		wrapTextBox fg bg = wrap ("^fg(" ++ bg ++ ")^i(" ++ myIconPath  ++ "boxleft.xbm)^bg(" ++ bg ++ ")^fg(" ++ fg ++ ")") ("^bg()^fg(" ++ bg ++ ")^i(" ++ myIconPath ++ "boxright.xbm)^fg()")
		--clickable config
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
	...
	, ((mod1Mask, xK_F3), spawn "opera")                                                       --Launch chromium
...
--	, ((0, xF86XK_iTouch), spawn "sh /home/nnoell/bin/touchpadtoggle.sh")                      --Toogle touchpad
